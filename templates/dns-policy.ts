...
type DNSPolicy = Record<string, string>;

const dnsPolicy = Object.entries(domainPolicy)
  .reduce((result: DNSPolicy, [key, value]) => {
    value.domains.forEach((domain) => {
      result[`'${domain}'`] = value.dns;
    });
    return result;
  }, {});

export default dnsPolicy;